...
// Désactive le cache pour garantir la mise à jour après chaque étape d'onboarding
...
export default async function DashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-white">
      {/* NOTE: Le header (SidebarTrigger, Breadcrumb) est déjà géré par votre 
          layout.tsx. On commence donc directement le contenu ici.
      */}
      ...
      {isOnboardingComplete ? (
        <div className="max-w-7xl mx-auto py-12 px-8 space-y-24 animate-in fade-in duration-700">
          ...
          {/* SECTION CONTENU : ANNONCES ET CTA */}
                  ...
                  <div key={item.id} className="group py-8 flex items-center justify-between transition-all">
                    <div className="space-y-1">
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-gray-300 font-light">#{item.id.slice(0, 5)}</span>
        ...
        /* VUE LORSQUE L'ONBOARDING N'EST PAS FINI */
...
// Composant local pour les blocs de statistiques
